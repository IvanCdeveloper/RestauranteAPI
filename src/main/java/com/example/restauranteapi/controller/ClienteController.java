...
//    }
    ...
    public ResponseEntity<List<ReservaDTO2>> findReservasByUsuario(@PathVariable String username) {
        Cliente cliente = clienteRepository.findByUsername(username)
                .orElseThrow(() -> new RuntimeException("Usuario no encontrado"));

        List<ReservaDTO2> reservasDTO = reservaRepository.findByCliente(cliente)
                .stream()
                .map(reserva -> ReservaDTO2.builder()
